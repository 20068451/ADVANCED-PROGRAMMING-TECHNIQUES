...
def onboard():
    ...
    if len(ppsn) != 9 or not ppsn.isdigit():
        return jsonify("PPSN must be exactly 9 digits."), 400
    try:
    ...
    today = date.today()
    if doj_date < today:
        return jsonify("Date of joining cannot be in the past."), 400
    with get_connection() as conn:
        cur = conn.cursor()
        ...
        cur.execute(
            ...
            (employee_id, name, date_of_joining, address, ppsn, position, department, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'ACTIVE')
            """,
...
def offboard():
    ...
    if not employee_id:
        return jsonify("employee_id is required"), 400
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            """
...
def monthly_report():
    ...
    if not year_str or not month_str:
        return jsonify("year and month are required"), 400
    try:
        year = int(year_str)
        month = int(month_str)
    except ValueError:
        return jsonify("year and month must be integers"), 400
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            """
